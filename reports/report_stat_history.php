...
$res = $rQ->statusHistoryReport($page,$loc);

$numRes = $rQ->numRes;

?>

    <h1>Ausleihhistorie</h1>
...
<?php ?>
    <table class="table table-hover table-bordered">
        ...
        while ($row = $rQ->fetchRowQ($res)){
            ...
            <td><a href='../circ/mbr_view.php?mbrid=".$row['mbrid']."'>".$row['mbrid']."</a></td><td>".statusToString($row['status_cd'])."</td>
            <td>".timestampToDate($row['status_begin_dt'])."</td><td>".timestampToDate($row['due_back_dt'])."</td></tr>";
        }
        ?>
    </table>

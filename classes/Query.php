<?php
/* This file is part of a copyrighted work; it is distributed with NO WARRANTY.
 * See the file COPYRIGHT.html for more details.
 */

$_Query_lock_depth = 0;

class Query
{
    var $_link;

    /**
     * Query constructor.
     * Constructing this class (or any of its child's) will automatically set up the Database connection
     */
    function Query()
    {
        static $link;
        if (!isset($link)) {
            if (!function_exists('mysqli_connect')) {
                Fatal::dbError("Checking for MySQL Extension...", "Unable to connect to database.", "The MySQL extension is not available");
            }

            $link = mysqli_connect(OBIB_HOST, OBIB_USERNAME, OBIB_PWD);
            if (!$link) {
                Fatal::dbError("Connecting to database server...", "Cannot connect to database server.", mysqli_error($link));
            }

            $rc = mysqli_select_db($link, OBIB_DATABASE);
            if (!$rc) {
                Fatal::dbError("Selecting database...", "Cannot select database.", mysqli_error($link));
            }
        }
        $this->_link = $link;
    }

    /**
     * Main Query function, this should be used and nothing else
     * @param string|$sql Query String
     * @return bool|mysqli_result - if error false, else mysqli_result
     */
    function queryDb($sql){
        if (!$this->_link) {
            Fatal::internalError('Tried to make database query before connection.');
        }
        $r = mysqli_query($this->_link, $sql);
        if ($r === false) {
            Fatal::dbError($sql, 'Database query failed', mysqli_error($this->_link));
        }
        return $r;
    }

    /**
     * Stupid old legacy stuff, should be removed/rewritten but works fine
     * @param $sql
     * @return array|bool|mysqli_result
     */
    function exec($sql)
    {
        $r = $this->queryDb($sql);
        if (is_bool($r)) {
            return $r;
        } else {
            $rows = array();
            while($row = mysqli_fetch_assoc($r)) {
                $rows[] = $row;
            }
            return $rows;
        }
    }

    /**
     * Runs mysqli_affected_rows on default link
     * @return int number of affected rows by last query
     */
    function affect(){
        if (!$this->_link) {
            Fatal::internalError('Tried to make database query before connection.');
        }
        return mysqli_affected_rows($this->_link);
    }

    //TODO refactor next 3 functions
    function select($sql)
    {
        $results = $this->queryDb($sql);
        if (is_bool($results)) {
            Fatal::dbError($sql, "Select did not return results.", 'No DBMS error.');
        }
        return new DbIter($results);
    }

    function select1($sql)
    {
        $r = $this->select($sql);
        if ($r->count() != 1) {
            Fatal::dbError($sql, 'Wrong number of result rows: expected 1, got ' . $r->count(),
                'No DBMS Error');
        } else {
            return $r->next();
        }
    }

    function select01($sql)
    {
        $r = $this->select($sql);
        if ($r->count() == 0) {
            return NULL;
        } else if ($r->count() != 1) {
            Fatal::dbError($sql, 'Wrong number of result rows: expected 0 or 1, got ' . $r->count(),
                'No DBMS Error');
        } else {
            return $r->next();
        }
    }

    /**
     * @return mixed - mysqli_insert_id on default link
     */
    function getInsertID()
    {
        return mysqli_insert_id($this->_link);
    }

    /* Locking functions
     *
     * Besides switching to InnoDB for transactions, I haven't been able to
     * come css with a good way to do locking reliably.  For now, we'll get
     * and release one big advisory lock around every sensitive transaction
     * and every database write except per-session data.  This should make
     * everything work, even if it is heavy-handed.
     *
     * Calls to lock/unlock may be nested, but must be paired.
     */
    function lock()
    {
        global $_Query_lock_depth;
        if ($_Query_lock_depth < 0) {
            Fatal::internalError('Negative lock depth');
        }
        if ($_Query_lock_depth == 0) {
            $row = $this->select1($this->mkSQL('select get_lock(%Q, %N) as locked',
                OBIB_LOCK_NAME, OBIB_LOCK_TIMEOUT));
            if (!isset($row['locked']) or $row['locked'] != 1) {
                Fatal::cantLock();
            }
        }
        $_Query_lock_depth++;
    }

    function unlock()
    {
        global $_Query_lock_depth;
        if ($_Query_lock_depth <= 0) {
            Fatal::internalError('Tried to unlock an unlocked database.');
        }
        $_Query_lock_depth--;
        if ($_Query_lock_depth == 0) {
            $this->queryDb($this->mkSQL('do release_lock(%Q)',
                OBIB_LOCK_NAME));
        }
    }

    /****************************************************************************
     * Makes SQL by interpolating values into a format string.
     * This function works something like printf() for SQL queries.  Format
     * strings contain %-escapes signifying that a value from the argument
     * list should be inserted into the string at that point.  The routine
     * properly quotes or transforms the value to make sure that it will be
     * handled correctly by the SQL server.  The recognized format strings
     * are as follows:
     *  %% - is replaced by a single '%' character and does not consume a
     *       value form the argument list.
     *  %! - inserts the argument in the query unaltered -- BE CAREFUL!
     *  %B - treates the argument as a boolean value and inserts either
     *       'Y' or 'N'as appropriate.
     *  %C - treats the argument as a column reference.  This differs from
     *       %I below only in that it passes the '.' operator for separating
     *       table and column names on to the SQL server unquoted.
     *  %I - treats the argument as an identifier to be quoted.
     *  %i - does the same escaping as %I, but does not add surrounding
     *       quotation marks.
     *  %N - treats the argument as a number and strips off all of it but
     *       an initial numeric string with optional sign and decimal point.
     *  %Q - treats the argument as a string and quotes it.
     *  %q - does the same escaping as %Q, but does not add surrounding
     *       quotation marks.
     * @param string $fmt format string
     * @param string ... optional argument values
     * @return string the result of interpreting $fmt
     * @access public
     ****************************************************************************
     */
    function mkSQL()
    {
        $n = func_num_args();
        if ($n < 1) {
            Fatal::internalError('Not enough arguments given to mkSQL().');
        }
        $i = 1;
        $SQL = "";
        $fmt = func_get_arg(0);
        while (strlen($fmt)) {
            $p = strpos($fmt, "%");
            if ($p === false) {
                $SQL .= $fmt;
                break;
            }
            $SQL .= substr($fmt, 0, $p);
            if (strlen($fmt) < $p + 2) {
                Fatal::internalError('Bad mkSQL() format string.');
            }
            if ($fmt{$p + 1} == '%') {
                $SQL .= "%";
            } else {
                if ($i >= $n) {
                    Fatal::internalError('Not enough arguments given to mkSQL().');
                }
                $arg = func_get_arg($i++);
                switch ($fmt{$p + 1}) {
                    case '!':
                        /* very dangerous, but sometimes very useful -- be careful */
                        $SQL .= $arg;
                        break;
                    case 'B':
                        if ($arg) {
                            $SQL .= "'Y'";
                        } else {
                            $SQL .= "'N'";
                        }
                        break;
                    case 'C':
                        $a = array();
                        foreach (explode('.', $arg) as $ident) {
                            array_push($a, '`' . $this->_ident($ident) . '`');
                        }
                        $SQL .= implode('.', $a);
                        break;
                    case 'I':
                        $SQL .= '`' . $this->_ident($arg) . '`';
                        break;
                    case 'i':
                        $SQL .= $this->_ident($arg);
                        break;
                    case 'N':
                        $SQL .= $this->_numstr($arg);
                        break;
                    case 'Q':
                        //switch arguments
                        //$SQL .= "'".mysqli_real_escape_string($arg, $this->_link)."'";
                        $SQL .= "'" . mysqli_real_escape_string($this->_link, $arg) . "'";
                        break;
                    case 'q':
                        //switch arguments
                        //$SQL .= "'".mysqli_real_escape_string($arg, $this->_link)."'";
                        $SQL .= "'" . mysqli_real_escape_string($this->_link, $arg) . "'";
                        break;
                    default:
                        Fatal::internalError('Bad mkSQL() format string.');
                }
            }
            $fmt = substr($fmt, $p + 2);
        }
        if ($i != $n) {
            Fatal::internalError('Too many arguments to mkSQL().');
        }
        return $SQL;
    }

    /**
     * Sanitizes Data (user input) to prevent SQL injection
     * @param $data string not sanitized data
     * @return string sanitized ("safe") data
     */
    function escape_data($data)
    {
        if (function_exists('mysqli_real_escape_string')) {
            $data = mysqli_real_escape_string($this->_link, trim($data));
            $data = strip_tags($data);
        } else {
            $data = mysqli_escape_string($this->_link, trim($data));
            $data = strip_tags($data);
        }

        return $data;
    }

    /**
     * Because the MySQL manual is unclear on how to include a ` in a `-quoted
     * identifier, we just drop them.  The manual does not say whether backslash
     * escapes are interpreted in quoted identifiers, so I assume they are not.
     * @param $i - input
     * @return mixed - input stripped from `
     */
    function _ident($i)
    {

        return str_replace('`', '', $i);
    }

    /**
     * @param $n
     * @return string
     */
    function _numstr($n)
    {
        if (preg_match("/^([+-]?[0-9]+(\.[0-9]*)?([Ee][0-9]+)?)/", $n, $subs)) {
            return $subs[1];
        } else {
            return "0";
        }
    }

    /**
     * New Query function for fetching rows, compatible with DbOld function but expects a result to be given
     * @param mysqli_result |$result - query result for row to be fetched
     * @param $arrayType - open biblio Array Type: OBID_NUM(fetch_row), OBID_BOTH(fetch_array),OBID_ASSOC(fetch_assoc)
     * @return bool | array - false if result is bool, array if result is valid
     */
    function fetchRowQ($result,$arrayType = OBIB_ASSOC)
    {
        if (is_bool($result)) {
            return false;
        }
        switch ($arrayType) {
            case OBIB_NUM:
                return mysqli_fetch_row($result);
                break;
            case OBIB_BOTH:
                return mysqli_fetch_array($result, MYSQLI_BOTH);
                break;
            case OBIB_ASSOC:
                return mysqli_fetch_assoc($result);
            default:
                return mysqli_fetch_assoc($result);
        }
    }

    /* Everything below is just a compatibility interface
     * for the last few iterations of this design.  Don't use
     * it.  This will be removed as soon as I get time to
     * update everything that depends on this stuff.
     *
     * TODO: what can be deleted
     */

    /**
     * @param bool $conn
     * @return bool
     * @deprecated true
     */
    function connect($conn = false)
    {
        return true;
    }

    /**
     * @return bool
     * @deprecated true
     */
    function close()
    {
        return true;
    }

    /**
     * @param string|$sql query method which also sets up DbOld
     * @param string [$msg] - deprecated parameter no longer used
     * @return bool|mysqli_result - query result
     * @deprecated function replaced by queryDb() which is still identical to this function
     */
    function _query($sql, $msg="")
    {
        $r = $this->queryDb($sql);
        return $r;
    }

    /**
     * @return bool
     * @deprecated true
     */
    function errorOccurred()
    {
        return false;
    }

}

/**
 * I have a feeling this should be deprecated too so i am deprecating it
 * @deprecated true
 */
class DbIter extends Iter
{
    function DbIter($results)
    {
        $this->results = $results;
    }

    function count()
    {
        return mysqli_num_rows($this->results);
    }

    function next()
    {
        $r = mysqli_fetch_assoc($this->results);
        if ($r === false) {
            return NULL;
        }
        return $r;
    }
}


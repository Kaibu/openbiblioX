...
class MemberQuery extends Query {
  ...
  function get($mbrid) {
    ...
    return $this->_mkObj($this->fetchRowQ($res));
  }
   ...
   * @return Member|bool returns library member or false if no more members to fetch
   * @access public
   ****************************************************************************
   */
  function fetchMember($res) {
  ...
  function getCustomFields($mbrid) {
    ...
    $rows = $q->fetchRowQ($q->queryDb($sql));
    $fields = array();
    foreach ($rows as $r) {
